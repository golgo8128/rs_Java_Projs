...
public class Build_metabPPI1_1 {

	private final RegServiceBag1_2 rSB;
	
	public Build_metabPPI1_1(RegServiceBag1_2 rSB){
		
		this.rSB = rSB;
		
	}
	
	public void build_net(){
		
	    // Create a new network
	    CyNetwork myNet = rSB.cyNetworkFactory.createNetwork();
	    // Set name for network
	    myNet.getRow(myNet).set(CyNetwork.NAME, "Test network 20140420 #1");
        
	    make_node_table(myNet,
	    		MetabPPI_NodeTable_Info1.clsnam_nodeattr,
	    		MetabPPI_NodeTable_Info1.colnam_nodeattr,
	    		MetabPPI_NodeTable_Info1.values_nodeattr);
	    	    
	    make_edge_table(myNet,
	    ...
	    rSB.cyNetworkManager.addNetwork(myNet);		
		
		CyNetworkView netview = rSB.networkViewFactory.createNetworkView(myNet);
